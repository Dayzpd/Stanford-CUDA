...
__global__ void shift_cypher(
  ...
  unsigned int alphabet_max,
  unsigned int array_length
) {
    // Calculate our unique global identifier for the current thread. This will
    // prevent any given thread from operating on incorrect data when it pulls
    // the next element from the input_array.
...
int main(void)
{
  // Uses the computer's internal clock to set the seed for the random number
  // generator.
  srand(time(NULL));
