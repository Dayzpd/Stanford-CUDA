...
void host_charged_particles(
  float4 *h_set_A,
  float4 *h_set_B,
  int *h_indices,
  float4 *h_force_vectors,
  int num_elements
) {
  ...
  int num_bytes_float4 = num_elements * sizeof(float4);
  int num_bytes_int = num_elements * sizeof(int);

  // cudaMalloc device arrays
  cudaMalloc((void**)&d_set_A, num_bytes_float4);
  cudaMalloc((void**)&d_set_B, num_bytes_float4);
  cudaMalloc((void**)&d_indices, num_bytes_int);
  cudaMalloc((void**)&d_force_vectors, num_bytes_float4);
  ...
  cudaMemcpy(d_set_A, h_set_A, num_bytes_float4, cudaMemcpyHostToDevice);
  cudaMemcpy(d_set_B, h_set_B, num_bytes_float4, cudaMemcpyHostToDevice);
  cudaMemcpy(d_indices, h_indices, num_bytes_int, cudaMemcpyHostToDevice);
  cudaMemcpy(d_force_vectors, h_force_vectors, num_bytes_float4, cudaMemcpyHostToDevice);
  ...
  // TODO more code here...
  cudaMemcpy(h_force_vectors, d_force_vectors, num_bytes_float4, cudaMemcpyDeviceToHost);
}
