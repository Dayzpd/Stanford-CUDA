...
bool cross_check_results(
  ...
  int * h_particles_binids_checker,
  int num_particles,
  int num_bins,
  int bin_size
) {
...
void device_binning(
  ...
  stop_timer(&timer, "Memory copy complete: ");

  start_timer(&timer);

  binning<<<num_particles / 256, 256>>>(
    d_particles,
    d_bins,
    d_bin_counters,
    gridding,
  ...
  stop_timer(&timer,"Device completed binning: ");
  ...
  stop_timer(&timer, "Finished copying memory from device to host: ");
...
int main(void)
{
  ...
  device_binning(
    h_particles, h_bins, h_bin_counters, gridding, num_particles, num_bins,
    bin_size
  );
  ...
  start_timer(&timer);
  host_binning(
    h_particles_checker, h_bins_checker, h_bin_counters_checker,
    &h_overflow_flag_checker, gridding, bin_size, num_particles
  );
  stop_timer(&timer,"Hosting binning completed: ");
