...
__global__ void device_find_knn(
  float3 *particles,
  int *knn,
  int num_particles,
  int num_neighbors
...
__global__ void device_find_knn_shared_mem(
  float3 *particles,
  int *knn,
  int num_particles,
  int num_neighbors
  ...
  if (i < num_particles)
  {
    ...
    init_list(&neigh_dist[threadIdx.x * num_neighbors],num_neighbors,2.0f);
    init_list(&neigh_ids[threadIdx.x * num_neighbors],num_neighbors,-1);
    for(int j=0;j<num_particles;j++)
    {
      if(i != j)
      {
        float rsq = dist2(p,particles[j]);
        insert_list(
          &neigh_dist[threadIdx.x * num_neighbors],
          &neigh_ids[threadIdx.x * num_neighbors],
          num_neighbors,
          rsq,
          j
        );
    ...
    for(int j=0;j<num_neighbors;j++)
    {
      knn[num_neighbors * i + j] = neigh_ids[threadIdx.x * num_neighbors + j];
    }
  }
}
  ...
  for(int i=0;i<num_particles;i++)
  {
    for(int j=0;j<num_neighbors;j++)
    {
      if(reference_knn[i*num_neighbors + j] != knn[i*num_neighbors + j])
      {
        ...
        {
          printf(
            "particle %d, neighbor %d is %d on cpu, %d on gpu\n",
            i,
            j,
            reference_knn[i*num_neighbors + j],
            knn[i*num_neighbors + j]
          );
        }
        error = 1;
      }
...
int main(void)
{
  ...
  stop_timer(&timer,"Copy memory from host to device: ");

  start_timer(&timer);
  device_find_knn<<<num_particles / BLOCK_SIZE, BLOCK_SIZE>>>(
    d_particles,
    d_knn,
    num_particles,
    num_neighbors
  ...
  start_timer(&timer);
  device_find_knn_shared_mem<<<num_particles / BLOCK_SIZE, BLOCK_SIZE>>>(
    d_particles,
    d_knn,
    num_particles,
    num_neighbors
  );
  check_cuda_error("shared mem knn");
  ...
  cudaMemcpy(
    h_knn,
    d_knn,
    num_particles * num_neighbors * sizeof(int),
