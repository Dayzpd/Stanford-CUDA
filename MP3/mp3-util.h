...
inline bool alt_fuzzy_validate(const real *x, const real *reference, const size_t n, const bool verbose = false)
{
  size_t num_mismatches = 0;
  ...
  {
    if(!almost_equal_relative(x[i + 1],reference[i], tolerance))
    {
      ++num_mismatches;
