...
__host__ __device__
void black_scholes(
  const real stock_price, const real option_strike, const real option_years,
  const real riskless_rate, const real volatility, real &call_result,
  real &put_result
) {
   real sqrt_option_years = sqrt(option_years);
   real d1 = (log(stock_price / option_strike) +
             (riskless_rate + 0.5 * volatility * volatility) * option_years) /
             (volatility * sqrt_option_years);
   ...
   put_result  = option_strike * expRT * (1.0 - CNDD2) -
                 stock_price * (1.0 - CNDD1);
}


void black_scholes_host(
  const real *d_stock_price, const real *d_option_strike,
  const real *d_option_years, real *d_call_result, real *d_put_result,
  const real riskless_rate, const real volatility, const size_t num_options
) {
  for(size_t i = 0; i < num_options; ++i)
  {
    // compute the call & put
    black_scholes(
      d_stock_price[i], d_option_strike[i], d_option_years[i], riskless_rate,
      volatility, d_call_result[i], d_put_result[i]
    );
  }
}

size_t filtered_black_scholes_host(
  const real *d_stock_price, const real *d_option_strike,
  const real *d_option_years, const real *d_previous_round_call_result,
  const real *d_previous_round_put_result, real *d_call_result,
  real *d_put_result, const real min_call_result, const real min_put_result,
  const real riskless_rate, const real volatility, const size_t num_options
) {
  size_t result = 0;
  for(size_t i = 0; i < num_options; ++i)
...
size_t compacted_black_scholes_host(
  const real *d_stock_price, const real *d_option_strike,
  const real *d_option_years, const real *d_previous_round_call_result,
  const real *d_previous_round_put_result, real *d_call_result,
  real *d_put_result, const real min_call_result, const real min_put_result,
...
__global__
void naively_filtered_black_scholes_kernel(
  const real *d_stock_price, const real *d_option_strike,
  const real *d_option_years, const real *d_previous_round_call_result,
  const real *d_previous_round_put_result, real *d_call_result,
  real *d_put_result, const real min_call_result, const real min_put_result,
