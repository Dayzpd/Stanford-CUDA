...
#include "black_scholes.cu"
#include "scan.cu"
#include "scatter.cu"
#include "compact.cu"
#include "mp3-util.h"

bool allocate_device_storage(
  real *&d_first_round_call_result, real *&d_first_round_put_result,
  real *&d_subsequent_round_call_result, real *&d_subsequent_round_put_result,
  real *&d_stock_price, real *&d_option_strike, real *&d_option_years,
  real *&d_compacted_stock_price, real *&d_compacted_option_strike,
  real *&d_compacted_option_years, const size_t num_options
) {
  cudaMalloc((void**)&d_first_round_call_result, num_options * sizeof(real));
  cudaMalloc((void**)&d_first_round_put_result, num_options * sizeof(real));
  cudaMalloc((void**)&d_subsequent_round_call_result, num_options * sizeof(real));
  cudaMalloc((void**)&d_subsequent_round_put_result, num_options * sizeof(real));
  cudaMalloc((void**)&d_stock_price, num_options * sizeof(real));
  cudaMalloc((void**)&d_option_strike, num_options * sizeof(real));
  cudaMalloc((void**)&d_option_years, num_options * sizeof(real));
  cudaMalloc((void**)&d_compacted_stock_price, num_options * sizeof(real));
  cudaMalloc((void**)&d_compacted_option_strike, num_options * sizeof(real));
  cudaMalloc((void**)&d_compacted_option_years, num_options * sizeof(real));
...
void deallocate_device_storage(
  ...
  cudaFree(d_compacted_option_years);
}
...
int main(void)
{
  ...
  const size_t num_subsequent_rounds = 50;
  ...
  start_timer(&timer);
  cudaMemcpy(
    d_stock_price, &h_stock_price[0], num_options * sizeof(real),
    cudaMemcpyHostToDevice
  );
  cudaMemcpy(
    d_option_strike, &h_option_strike[0], num_options * sizeof(real),
    cudaMemcpyHostToDevice
  );
  cudaMemcpy(
    d_option_years, &h_option_years[0], num_options * sizeof(real),
    cudaMemcpyHostToDevice
  );
  ...
  start_timer(&timer);
  black_scholes_kernel<<<num_options / BLOCK_SIZE, BLOCK_SIZE>>>(
    &d_stock_price[0], &d_option_strike[0], &d_option_years[0],
    &d_first_round_call_result[0], &d_first_round_put_result[0],
    first_round_riskless_rate, first_round_volatility, num_options
  );
  ...
  num_compacted_options = compact_options(
    min_call_threshold, min_put_threshold, d_first_round_call_result,
    d_first_round_put_result, &d_stock_price[0], &d_option_strike[0],
    &d_option_years[0], num_options, d_compacted_stock_price,
    d_compacted_option_strike, d_compacted_option_years
  );
  ...
  for(int round = 1; round <= num_subsequent_rounds; round++)
  {
    ...
    black_scholes_kernel<<<num_options / BLOCK_SIZE, BLOCK_SIZE>>>(
                                 ...
                                 d_compacted_option_strike,
                                 d_compacted_option_years,
                                 &d_subsequent_round_call_result[0],
                                 &d_subsequent_round_put_result[0],
                                 riskless_rate,
                                 volatility,
                                 num_options);
    char message[256];
    ...
    if(!alt_fuzzy_validate(&h_validate_me[0], &h_subsequent_round_call_result[0], num_compacted_options_reference))
    {
    ...
    if(!alt_fuzzy_validate(&h_validate_me[0], &h_subsequent_round_put_result[0], num_compacted_options_reference))
    {
