                                       ...
                                       real *d_option_years_result)
{
  ...
  for(; i < n; i += grid_size)
  {
    ...
    if((call >= min_call_threshold) && (put >= min_put_threshold))
    {
      ...
      //printf("index: %d, price: %f, strike: %f, years: %f\n", d_scatter_indices[i], d_stock_price_result[d_scatter_indices[i]],
      //  d_option_strike_result[d_scatter_indices[i]], d_option_years_result[d_scatter_indices[i]]);
    }
  }
  __syncthreads();
}
