...
int main(void)
{
  ...
  start_timer(&timer);
  black_scholes_kernel<<<num_options / 1024, 1024>>>(
  ...
  start_timer(&timer);
  black_scholes_host(
    &h_stock_price[0], &h_option_strike[0], &h_option_years[0],
    &h_first_round_call_result[0], &h_first_round_put_result[0],
    first_round_riskless_rate, first_round_volatility, num_options
  );
  ...
  cudaMemcpy(
    &h_validate_me[0], d_first_round_call_result, sizeof(real) * num_options,
    cudaMemcpyDeviceToHost
  );
  // pass true as a final optional argument to fuzzy_validate for verbose output
  if(!fuzzy_validate(
    &h_validate_me[0], &h_first_round_call_result[0], num_options, true
  )) {
  ...
  cudaMemcpy(
    &h_validate_me[0], d_first_round_put_result, sizeof(real) * num_options,
    cudaMemcpyDeviceToHost
  );
  ...
  for(int round = 1; round < num_subsequent_rounds + 1; ++round)
  {
    ...
    start_timer(&timer);
    naively_filtered_black_scholes_kernel<<<num_options / 1024, 1024>>>(
      &d_stock_price[0], &d_option_strike[0], &d_option_years[0],
      &d_first_round_call_result[0], &d_first_round_put_result[0],
      &d_subsequent_round_call_result[0], &d_subsequent_round_put_result[0],
      min_call_threshold, min_put_threshold, riskless_rate, volatility,
      num_options
    );
    char message[256];
    ...
    start_timer(&timer);
    num_filtered_options = filtered_black_scholes_host(
      &h_stock_price[0], &h_option_strike[0], &h_option_years[0],
      &h_first_round_call_result[0], &h_first_round_put_result[0],
      &h_subsequent_round_call_result[0], &h_subsequent_round_put_result[0],
      min_call_threshold, min_put_threshold, riskless_rate, volatility,
      num_options
    );
    ...
    cudaMemcpy(
      &h_validate_me[0], d_subsequent_round_call_result,
      sizeof(real) * num_options, cudaMemcpyDeviceToHost
    );
  ...
  deallocate_device_storage(
    d_first_round_call_result, d_first_round_put_result,
    d_subsequent_round_call_result, d_subsequent_round_put_result,
    d_stock_price, d_option_strike, d_option_years
  );
